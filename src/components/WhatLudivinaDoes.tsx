    ...
    <section className="bg-background border-t border-border py-24">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="font-cinzel text-3xl md:text-4xl tracking-wider text-foreground mb-6">
            Canal entre el cielo y tu alma
          </h2>
          <p className="font-outfit text-lg text-muted-foreground leading-relaxed font-light tracking-wide">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-center max-w-4xl mx-auto">
          {stats.map((stat) => (
            <div key={stat.label}>
              <p className="font-cinzel text-4xl md:text-5xl text-gold tracking-wider mb-2">{stat.number}</p>
              <p className="font-cinzel font-semibold text-xs tracking-[0.2em] uppercase text-muted-foreground">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
